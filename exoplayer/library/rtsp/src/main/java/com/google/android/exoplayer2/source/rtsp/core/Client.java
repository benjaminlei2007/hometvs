...
//            mode = RTSP_INTERLEAVED;
    ...
    @Override
    public final void onMoved(Response response){
        Uri uri=Uri.parse(response.getHeaders().value(Header.Location));
        this.close();
        this.uri=uri;
        dispatcher = new Dispatcher.Builder(this)
                .setUri(uri)
                ...
                if ((content != null) && (content.length() > 0)) {
                    ...
                    for (Attribute attribute : sessionDescription.attributes()) {
                        ...
                        if (Attribute.RANGE.equalsIgnoreCase(attrName)) {
                            session.setRange(Range.parse(attribute.value()));
                        ...
                        for (MediaDescription mediaDescription :
                            ...
                            if ((Media.audio.equals(media.type()) && !isAVOptionSet(
                                ...
                                for (Attribute attribute : mediaDescription.attributes()) {
                                    ...
                                    if (Attribute.RANGE.equalsIgnoreCase(attrName)) {
                                        session.setRange(Range.parse(attrValue));
    ...
    public abstract void sendPlayRequest(String rangeStr);
    public abstract void sendPlayRequest(String rangeStr, float scale);
